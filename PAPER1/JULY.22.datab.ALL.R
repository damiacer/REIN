...
tab.one <- CreateTableOne(vars = variables, data = rdb, factorVars = categorical, includeNA = TRUE)
...
rdb.toimput$dial <- as.factor(rdb$dial)
...
rdb.toimput$diabetes <- as.factor(rdb.toimput$diabetes)
...
# dial
cox.t2.dial = with(rdb.imputed, coxph(Surv(time, status=="1") ~ dial))
...
cox.t3 = with(rdb.imputed, coxph(Surv(time, status=="1") ~ cardiovasc + tabac2 +
                                   dial + apkd01 +
...
#################################################################################
rdb.toimputd = subset(rdb, select = c(cardiovasc, tabac2, dial, apkd01, 
                                     sex, age, diabetes, bmic, status, time))

rdb.toimputd$cardiovasc <- as.factor(rdb.toimputd$cardiovasc)
rdb.toimputd$tabac2 <- as.factor(rdb.toimputd$tabac2)
rdb.toimputd$dial <- as.factor(rdb$dial)
rdb.toimputd$apkd01 <- as.factor(rdb.toimputd$apkd01)
rdb.toimputd$sex <- as.factor(rdb.toimputd$sex)
rdb.toimputd$age <- as.numeric(as.character(rdb.toimputd$age))
rdb.toimputd$diabetes <- as.numeric(as.character(rdb.toimputd$diabetes))
rdb.toimputd$status <- as.factor(rdb.toimputd$status)
rdb.toimputd$time <- as.numeric(as.character(rdb.toimputd$time))
rdb.toimputd$bmic <- as.factor(rdb.toimputd$bmic)

rdb.toimputd$diabetes[is.na(rdb.toimputd$diabetes)] <- 2

table(rdb.toimputd$diabetes)
# 0     1     2 
# 22943 20156  1927 

rdb.toimput.diab = rdb.toimputd[(rdb.toimputd$diabetes == 0),] # this will NOT includes the NA

count(rdb.toimput.diab) # 22943

rdb.toimputd = subset(rdb.toimputd, select = -c(diabetes))

table(rdb.toimput.diab$status)
...
# dial
cox.t4.dial = with(rdbdia.imputed, coxph(Surv(time, status=="1") ~ dial))
summary(pool(cox.t4.dial))
est.dial <- pool(cox.t4.dial)
...
cox.t5 = with(rdbdia.imputed, coxph(Surv(time, status=="1") ~ cardiovasc + tabac2 + dial + apkd01 +
                                      bmic + sex + age))
summary(pool(cox.t5))
est.t5 <- pool(cox.t5)

...
rdb.toimput.h$status <- as.factor(rdb.toimput.h$status)
rdb.toimput.h$time <- as.numeric(as.character(rdb.toimput.h$time))
rdb.toimput.h$bmic <- as.factor(rdb.toimput.h$bmic)

table(rdb.toimput.h$status)

rdbischemic.hmputed<- mice(rdb.toimput.h, m=20, maxit=10, meth='pmm', seed=210586,
                           print = FALSE)

#--------------------------------------------------------------------------------
# TABLE 6
#table6: bivariate analysis for haemorrhagic stroke, all patients

# cardiovascular
cox.t8.cardiovasc = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ cardiovasc))
summary(pool(cox.t8.cardiovasc))
est.card <- pool(cox.t8.cardiovasc)
summary(est.card, conf.hnt = TRUE, exponentiate = TRUE)

# tabac2
cox.t8.tab = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ tabac2))
summary(pool(cox.t8.tab))
est.tab <- pool(cox.t8.tab)
summary(est.tab, conf.hnt = TRUE, exponentiate = TRUE)

# tabac2
cox.t8.dial = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ dial))
summary(pool(cox.t8.dial))
est.dial <- pool(cox.t8.dial)
summary(est.dial, conf.hnt = TRUE, exponentiate = TRUE)

# adpkd
cox.t8.adpkd = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ apkd01))
summary(pool(cox.t8.adpkd))
est.adpkd <- pool(cox.t8.adpkd)
summary(est.adpkd, conf.hnt = TRUE, exponentiate = TRUE)

# bmic
cox.t8.bmic = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ bmic))
summary(pool(cox.t8.bmic))
est.bmic <- pool(cox.t8.bmic)
summary(est.bmic, conf.hnt = TRUE, exponentiate = TRUE)

# sex
cox.t8.sex = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ sex))
summary(pool(cox.t8.sex))
est.sex <- pool(cox.t8.sex)
summary(est.sex, conf.hnt = TRUE, exponentiate = TRUE)

# age
cox.t8.age = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ age))
summary(pool(cox.t8.age))
est.age <- pool(cox.t8.age)
summary(est.age, conf.hnt = TRUE, exponentiate = TRUE)

# diabetes
cox.t8.dia = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ diabetes))
summary(pool(cox.t8.dia))
est.dia <- pool(cox.t8.dia)
summary(est.dia, conf.hnt = TRUE, exponentiate = TRUE)

#--------------------------------------------------------------------------------

# TABLE 7
# table7: multivariate analysis for haemorrhagic stroke, all patients

cox.t7 = with(rdbischemic.hmputed, coxph(Surv(time, status=="1") ~ cardiovasc + tabac2 + dial + 
                                           apkd01 +
                                           bmic + sex + age + diabetes))
summary(pool(cox.t7))
est.t7 <- pool(cox.t7)
summary(est.t7, conf.hnt = TRUE, exponentiate = TRUE)

#--------------------------------------------------------------------------------
# DATASET FOR HAEMORRHAGIC ONLY EVENTS IN NON-DIABETIC POPULATION

rdb.toimput.hnd = subset(rdb, select = c(cardiovasc, tabac2, dial, apkd01, 
                                         sex, age, diabetes, bmic, status, time))

rdb.toimput.hnd$cardiovasc <- as.factor(rdb.toimput.hnd$cardiovasc)
rdb.toimput.hnd$tabac2 <- as.factor(rdb.toimput.hnd$tabac2)
rdb.toimput.hnd$apkd01 <- as.factor(rdb.toimput.hnd$apkd01)
rdb.toimput.hnd$sex <- as.factor(rdb.toimput.hnd$sex)
rdb.toimput.hnd$age <- as.numeric(as.character(rdb.toimput.hnd$age))
rdb.toimput.hnd$diabetes <- as.numeric(as.character(rdb.toimput.hnd$diabetes))
rdb.toimput.hnd$status <- as.factor(rdb.toimput.hnd$status)
rdb.toimput.hnd$time <- as.numeric(as.character(rdb.toimput.hnd$time))
rdb.toimput.hnd$bmic <- as.factor(rdb.toimput.hnd$bmic)

# DIABETES VARIABLE 
rdb.toimput.hnd$diabetes[is.na(rdb.toimput.hnd$diabetes)] <- 2
table(rdb.toimput.hnd$diabetes)
#     0     1     2 
# 22943 20156  1927 

rdb.toimput.hnddiab = rdb.toimput.hnd[(rdb.toimput.hnd$diabetes==0),] 
count(rdb.toimput.hnddiab) # 22943

rdb.toimput.hnddiab = subset(rdb.toimput.hnddiab, select = -c(diabetes))

rdbdia.hnd.hmputed<- mice(rdb.toimput.hnddiab, m=20, maxit=10, meth='pmm', seed=210586,
                          print = FALSE)

#--------------------------------------------------------------------------------
# TABLE 8
# table8: bivariate analysis for haemorrhagic stroke, non diabetic patients

# cardiovascular
cox.t8.cardiovasc = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ cardiovasc))
summary(pool(cox.t8.cardiovasc))
est.card <- pool(cox.t8.cardiovasc)
summary(est.card, conf.hnt = TRUE, exponentiate = TRUE)

# tabac2
cox.t8.tab = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ tabac2))
summary(pool(cox.t8.tab))
est.tab <- pool(cox.t8.tab)
summary(est.tab, conf.hnt = TRUE, exponentiate = TRUE)
...
cox.t8.dial = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ dial))
summary(pool(cox.t8.dial))
est.dial <- pool(cox.t8.dial)
summary(est.dial, conf.hnt = TRUE, exponentiate = TRUE)

# adpkd
cox.t8.adpkd = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ apkd01))
summary(pool(cox.t8.adpkd))
est.adpkd <- pool(cox.t8.adpkd)
summary(est.adpkd, conf.hnt = TRUE, exponentiate = TRUE)

# bmic
cox.t8.bmic = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ bmic))
summary(pool(cox.t8.bmic))
est.bmic <- pool(cox.t8.bmic)
summary(est.bmic, conf.hnt = TRUE, exponentiate = TRUE)

# sex
cox.t8.sex = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ sex))
summary(pool(cox.t8.sex))
est.sex <- pool(cox.t8.sex)
summary(est.sex, conf.hnt = TRUE, exponentiate = TRUE)

# age
cox.t8.age = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ age))
summary(pool(cox.t8.age))
est.age <- pool(cox.t8.age)
summary(est.age, conf.hnt = TRUE, exponentiate = TRUE)
...
cox.t9 = with(rdbdia.hnd.hmputed, coxph(Surv(time, status=="1") ~ cardiovasc + tabac2 + dial + apkd01 +
                                          bmic + sex + age))
summary(pool(cox.t9))
est.t9 <- pool(cox.t9)
summary(est.t9, conf.hnt = TRUE, exponentiate = TRUE)

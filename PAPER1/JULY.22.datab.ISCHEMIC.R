...
# TABLES 10-13: ALL ISCHEMIC EVENTS 
...
# dial
cox.t8.dial = with(rdbischemic.imputed, coxph(Surv(time, status=="1") ~ dial))
...
rdb.toimput.ind$dial <- as.factor(rdb.toimput.ind$dial)
...
count(rdb.toimput.inddiab) # 22943

table(rdb.toimput.inddiab$status)

rdb.toimput.inddiab = subset(rdb.toimput.inddiab, select = -c(diabetes))
...
# dial
cox.t8.dial = with(rdbdia.ind.imputed, coxph(Surv(time, status=="1") ~ dial))
summary(pool(cox.t8.dial))
est.dial <- pool(cox.t8.dial)
summary(est.dial, conf.int = TRUE, exponentiate = TRUE)
...
cox.t9 = with(rdbdia.ind.imputed, coxph(Surv(time, status=="1") ~ cardiovasc + tabac2 + 
                                          dial + apkd01 +
                                          bmic + sex + age))
summary(pool(cox.t9))
est.t9 <- pool(cox.t9)
